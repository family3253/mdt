...
### 6) Codex 本地配置冲突（401 专项排查）
- 若出现“登录成功但调用 401”，优先检查本机遗留配置：
  - `~/.codex/config.toml`（可能覆盖 provider/base_url 或 API key）
  - `~/.codex/auth.json`（缓存凭证）
- 处理步骤：
  1) 备份并移走 `config.toml`
  2) 删除 `auth.json`
  3) 重启相关客户端/服务后再验证
- 该类 401 属于“本地配置覆盖/冲突”，不要误判为账号池整体失效。
