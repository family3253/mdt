...
def _find_missing_modules(modules: tuple[str, ...]) -> List[str]:
    missing: List[str] = []
...
def _run_pip_install(command: List[str], cwd: Path) -> bool:
    cmd_text = " ".join(command)
    print(f"[deps] 执行安装命令: {cmd_text}")
    result = subprocess.run(command, cwd=str(cwd), check=False)
    print(f"[deps] 安装命令退出码: {result.returncode}")
    return result.returncode == 0


def ensure_runtime_dependencies() -> None:
    required_modules: tuple[str, ...] = ("requests", "urllib3", "aiohttp")
    missing = _find_missing_modules(required_modules)
    if not missing:
        return

    print(f"[deps] 检测到缺少依赖: {', '.join(missing)}")
    script_dir = Path(__file__).resolve().parent
    requirements_path = script_dir / "requirements.txt"
    python_exe = sys.executable or "python"

    if requirements_path.exists():
        print(f"[deps] 优先使用 requirements.txt 安装依赖: {requirements_path}")
        _run_pip_install(
            [python_exe, "-m", "pip", "install", "-r", str(requirements_path)],
            script_dir,
        )
    else:
        print("[deps] 未找到 requirements.txt，回退安装 requests urllib3 aiohttp")
        _run_pip_install(
            [python_exe, "-m", "pip", "install", "requests", "urllib3", "aiohttp"],
            script_dir,
        )

    missing_after_install = _find_missing_modules(required_modules)
...
ALLOWED_EMAIL_EXPIRY_VALUES = {3600000, 86400000, 604800000, 0}
DEFAULT_EMAIL_EXPIRY = 3600000
...
def mask_proxy_for_log(proxy: str) -> str:
    ...
    try:
        ...
        new_netloc = f"{safe_auth}@{host_part}"
        return parsed._replace(netloc=new_netloc).geturl()
    except Exception:
        return p


def resolve_proxy_settings(conf: Dict[str, Any]) -> Dict[str, str]:
    all_proxy = normalize_proxy_url(
        str(
            pick_conf(
                conf,
                "proxy",
                ...
                "proxy",
                "all_proxy",
                default=pick_conf(conf, "run", "proxy", default=""),
            )
            or ""
        )
    )
    http_proxy = normalize_proxy_url(
        str(
            pick_conf(
                ...
                "http",
                "http_proxy",
                default=pick_conf(conf, "run", "http_proxy", default=""),
            )
            or ""
        )
    )
    https_proxy = normalize_proxy_url(
        str(
            pick_conf(
                ...
                "https",
                "https_proxy",
                default=pick_conf(conf, "run", "https_proxy", default=""),
            )
            or ""
        )
    )
    no_proxy = str(
        pick_conf(
            ...
            "proxy",
            "no_proxy",
            default=pick_conf(conf, "run", "no_proxy", default=""),
        )
        or ""
    ).strip()
...
def apply_proxy_environment(
    ...
    proxy_settings = resolve_proxy_settings(conf)

    all_proxy = proxy_settings.get("all", "")
    ...
    if all_proxy or http_proxy or https_proxy:
        logger.info(
            "已启用代理: all=%s http=%s https=%s no_proxy=%s",
            mask_proxy_for_log(all_proxy) or "-",
            mask_proxy_for_log(http_proxy) or "-",
            mask_proxy_for_log(https_proxy) or "-",
            no_proxy or "-",
        )
    else:
...
def pick_runtime_proxy(conf: Dict[str, Any]) -> str:
    proxy_settings = resolve_proxy_settings(conf)
    return (
        proxy_settings.get("https")
        or proxy_settings.get("http")
        or proxy_settings.get("all")
        or ""
    )
...
class SentinelTokenGenerator:
    ...
    def generate_token(
        self, seed: Optional[str] = None, difficulty: Optional[str] = None
    ) -> str:
        if seed is None:
...
def normalize_email_base(value: str) -> str:
...
def moemail_headers(api_key: str) -> Dict[str, str]:
    return {
        "X-API-Key": api_key,
        "Accept": "application/json",
        "Content-Type": "application/json",
...
def fetch_moemail_domains(
    session: requests.Session, email_api_base: str, api_key: str
) -> List[str]:
    if not email_api_base or not api_key:
        return []

    try:
        res = session.get(
            f"{email_api_base}/api/config",
            headers=moemail_headers(api_key),
            timeout=15,
            verify=False,
        )
        ...
        raw_domains = data.get("emailDomains")
        if isinstance(raw_domains, list):
            return [str(x).strip() for x in raw_domains if str(x).strip()]
        if isinstance(raw_domains, str):
            return [x.strip() for x in raw_domains.split(",") if x.strip()]
    except Exception:
        pass
    return []


def build_mail_raw_text(item: Dict[str, Any]) -> str:
    parts: List[str] = []
    for key in ("raw", "content", "text", "subject", "html", "body"):
        val = item.get(key)
        if val:
...
def fetch_moemail_message_detail(
    ...
    if not (email_api_base and mailbox_id and message_id and api_key):
        return {}

    try:
        res = session.get(
            f"{email_api_base}/api/emails/{quote(mailbox_id, safe='')}/{quote(message_id, safe='')}",
            headers=moemail_headers(api_key),
            timeout=30,
            verify=False,
        )
...
def create_temp_email(
    ...
    domain_pool = [str(x).strip() for x in email_domains if str(x).strip()]
    if not domain_pool:
        domain_pool = fetch_moemail_domains(session, email_api_base, api_key)

    chosen_domain = random.choice(domain_pool) if domain_pool else ""
    payload: Dict[str, Any] = {"name": name, "expiryTime": int(expiry_time)}
    if chosen_domain:
        payload["domain"] = chosen_domain

    attempts = max(1, int(retry_attempts or 1))
    backoff_base = float(retry_backoff_base or 1.8)
    backoff_max = max(0.1, float(retry_backoff_max or 12.0))
    retryable_status_codes = {408, 425, 429, 500, 502, 503, 504}

    for attempt in range(1, attempts + 1):
        try:
            res = session.post(
                f"{email_api_base}/api/emails/generate",
                json=payload,
                headers=moemail_headers(api_key),
                timeout=30,
                verify=False,
            ...
            if attempt >= attempts:
                logger.warning("创建 moemail 邮箱异常(重试耗尽): %s", e)
                return None, None
            backoff = min(backoff_max, backoff_base ** (attempt - 1))
            jitter = random.uniform(0.2, 0.8)
            wait_seconds = backoff + jitter
            logger.warning(
                "创建 moemail 邮箱重试 %s/%s: 异常=%s, 下次等待 %.2fs",
                attempt,
                attempts,
                e,
                wait_seconds,
        ...
        if res.status_code == 200:
            ...
            mailbox_id = (
                data.get("id") or data.get("mailboxId") or data.get("mailbox_id")
            )
            if email and mailbox_id:
                logger.info(
                    "创建 moemail 邮箱成功: %s (id=%s, domain=%s)",
                    email,
                    mailbox_id,
                    chosen_domain or "auto",
                )
                return str(email), str(mailbox_id)
            logger.warning("创建邮箱响应缺少 email 或 id: %s", str(data)[:300])
            return None, None

        if res.status_code not in retryable_status_codes:
            logger.warning(
                "创建 moemail 邮箱失败: HTTP %s, 响应=%s",
                res.status_code,
                (res.text or "")[:300],
            )
            return None, None
        ...
        if attempt >= attempts:
            logger.warning(
                "创建 moemail 邮箱失败(重试耗尽): HTTP %s, 响应=%s",
                res.status_code,
                (res.text or "")[:300],
            )
            return None, None
        ...
        wait_seconds = backoff + jitter
        logger.warning(
            "创建 moemail 邮箱重试 %s/%s: HTTP %s, 下次等待 %.2fs",
            attempt,
            attempts,
            res.status_code,
            wait_seconds,
...
def fetch_emails(
    ...
    if not (email_api_base and mailbox_id and api_key):
        return []

    try:
        res = session.get(
            f"{email_api_base}/api/emails/{quote(mailbox_id, safe='')}",
            headers=moemail_headers(api_key),
            timeout=30,
            verify=False,
        )
        ...
        for idx, item in enumerate(messages[: max(1, limit)]):
            if not isinstance(item, dict):
                continue

            msg_id = str(item.get("id") or "")
            raw = build_mail_raw_text(item)
            if not raw and msg_id:
                detail = fetch_moemail_message_detail(
                    session, email_api_base, mailbox_id, msg_id, api_key
                )
                raw = build_mail_raw_text(detail)

            if msg_id or raw:
                rows.append({"id": msg_id or f"{idx}", "raw": raw})
...
def wait_for_verification_code(
    ...
    old_ids = set()
    old = fetch_emails(session, email_api_base, mailbox_id, api_key)
    if old:
    ...
    while time.time() - start < timeout:
        emails = fetch_emails(session, email_api_base, mailbox_id, api_key)
        if emails:
            for item in emails:
                if not isinstance(item, dict):
                    continue
...
class ProtocolRegistrar:
    ...
    def _build_headers(
        self, referer: str, with_sentinel: bool = False
    ) -> Dict[str, str]:
    ...
    def step0_init_oauth_session(
        self, email: str, client_id: str, redirect_uri: str
    ) -> bool:
    ...
    def step5_create_account(
        self, first_name: str, last_name: str, birthdate: str
    ) -> bool:
        ...
        try:
            ...
            if r.status_code == 403 and "sentinel" in r.text.lower():
                self.logger.warning(
                    "步骤5告警: create_account 命中sentinel风控，尝试重试"
                )
                h["openai-sentinel-token"] = SentinelTokenGenerator(
                    self.device_id
                ).generate_token()
...
def perform_codex_oauth_login_http(
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        if not mailbox_id or not email_api_key:
            return None
        ...
        if "about-you" in continue_url:
            ...
            if "consent" in str(resp_about.url) or "organization" in str(
                resp_about.url
            ):
    ...
    def _follow_and_extract_code(
        session_obj: requests.Session, url: str, max_depth: int = 10
    ) -> Optional[str]:
        if max_depth <= 0:
            return None
        try:
...
class RegisterRuntime:
    def __init__(
        self, conf: Dict[str, Any], target_tokens: int, logger: logging.Logger
    ):
        self.conf = conf
        ...
        self.proxy = pick_runtime_proxy(conf)
        ...
        self.email_api_base = normalize_email_base(email_base_raw)
        ...
        if not parsed_domains:
            parsed_domains = [old_domain] if old_domain else []
        self.email_domains = parsed_domains

        self.email_api_key = str(
            pick_conf(conf, "email", "api_key", "admin_password", default="") or ""
        )

        expiry_raw = pick_conf(
            conf,
            "email",
            "expiry_time",
            "expiry_time_ms",
            default=DEFAULT_EMAIL_EXPIRY,
        )
        try:
            parsed_expiry = int(expiry_raw)
            if parsed_expiry not in ALLOWED_EMAIL_EXPIRY_VALUES:
                self.logger.warning(
                    "email.expiry_time=%s 非 moemail 推荐值，已回退为默认 %s。允许值: %s",
                    parsed_expiry,
                    DEFAULT_EMAIL_EXPIRY,
                    sorted(ALLOWED_EMAIL_EXPIRY_VALUES),
                )
                self.email_expiry_time = DEFAULT_EMAIL_EXPIRY
            else:
                self.email_expiry_time = parsed_expiry
        except Exception:
            self.email_expiry_time = DEFAULT_EMAIL_EXPIRY

        retry_attempts_raw = pick_conf(
            conf,
            "email",
        ...
        try:
            self.email_create_retry_attempts = max(1, int(retry_attempts_raw))
        except Exception:
            self.email_create_retry_attempts = 5

        retry_backoff_base_raw = pick_conf(
            conf,
            "email",
        ...
        try:
            self.email_create_retry_backoff_base = max(
                1.0, float(retry_backoff_base_raw)
            )
        except Exception:
            self.email_create_retry_backoff_base = 1.8

        retry_backoff_max_raw = pick_conf(
            conf,
            "email",
            "create_retry_backoff_max",
            default=12,
        )
        try:
            self.email_create_retry_backoff_max = max(0.5, float(retry_backoff_max_raw))
        except Exception:
            self.email_create_retry_backoff_max = 12.0

        self.oauth_issuer = str(
            pick_conf(conf, "oauth", "issuer", default="https://auth.openai.com")
            or "https://auth.openai.com"
        )
        self.oauth_client_id = str(
            pick_conf(
                conf, "oauth", "client_id", default="app_EMoamEEZ73f0CkXaXp7hrann"
            )
        ...
        self.oauth_retry_attempts = int(
            pick_conf(conf, "oauth", "retry_attempts", default=3) or 3
        )
        self.oauth_retry_backoff_base = float(
            pick_conf(conf, "oauth", "retry_backoff_base", default=2.0) or 2.0
        )
        self.oauth_retry_backoff_max = float(
            pick_conf(conf, "oauth", "retry_backoff_max", default=15.0) or 15.0
        )

        upload_base = str(
            pick_conf(conf, "upload", "cli_proxy_api_base", "base_url", default="")
            or ""
        ).strip()
        if not upload_base:
            upload_base = str(
                pick_conf(conf, "clean", "base_url", default="") or ""
            ).strip()
        self.cli_proxy_api_base = upload_base.rstrip("/")

        upload_token = str(
            pick_conf(conf, "upload", "token", "cpa_password", default="") or ""
        ).strip()
        if not upload_token:
            upload_token = str(
        ...
        self.upload_url = (
            f"{self.cli_proxy_api_base}/v0/management/auth-files"
            if self.cli_proxy_api_base
            else ""
        )
        ...
        if isinstance(save_local_raw, bool):
            ...
            self.save_local = str(save_local_raw).strip().lower() in (
                "1",
                "true",
                "yes",
                "on",
        ...
        if self.save_local:
            ...
            self.tokens_out_dir = self._ensure_unique_dir(
                self.tokens_parent_dir, f"{target_tokens}个账号"
            )

            self.accounts_file = self._resolve_output_path(
                str(output_cfg.get("accounts_file", "accounts.txt"))
            )
            self.csv_file = self._resolve_output_path(
                str(output_cfg.get("csv_file", "registered_accounts.csv"))
            )
            self.ak_file = self._resolve_output_path(
                str(output_cfg.get("ak_file", "ak.txt"))
            )
            self.rk_file = self._resolve_output_path(
                str(output_cfg.get("rk_file", "rk.txt"))
            )
        else:
            self.fixed_out_dir = ""
    ...
    def save_token_json(
        self, email: str, access_token: str, refresh_token: str = "", id_token: str = ""
    ) -> bool:
        try:
            ...
            if exp_timestamp:
                exp_dt = dt.datetime.fromtimestamp(
                    exp_timestamp, tz=dt.timezone(dt.timedelta(hours=8))
                )
    ...
    def upload_token_json(self, filename: str) -> None:
        ...
        try:
            ...
            with open(filename, "rb") as f:
                ...
                if resp.status_code != 200:
                    self.logger.warning(
                        "上传 token 失败: %s %s", resp.status_code, resp.text[:200]
                    )
        except Exception as e:
    ...
    def upload_token_data(self, filename: str, token_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {self.upload_api_token}"}
            resp = s.post(
                self.upload_url, files=files, headers=headers, verify=False, timeout=30
            )
            if resp.status_code != 200:
                self.logger.warning(
                    "上传 token 失败: %s %s", resp.status_code, resp.text[:200]
    ...
    def oauth_login_with_retry(
        self, email: str, password: str, mailbox_id: str
    ) -> Optional[Dict[str, Any]]:
        attempts = max(1, self.oauth_retry_attempts)
        for attempt in range(1, attempts + 1):
            if (
                self.stop_event.is_set()
                and self.get_token_success_count() >= self.target_tokens
            ):
                return None
            ...
            tokens = perform_codex_oauth_login_http(
                ...
                email_api_base=self.email_api_base,
            ...
            if attempt < attempts:
                backoff = min(
                    self.oauth_retry_backoff_max,
                    self.oauth_retry_backoff_base ** (attempt - 1),
                )
                jitter = random.uniform(0.2, 0.8)
...
def register_one(
    runtime: RegisterRuntime, worker_id: int = 0
) -> tuple[Optional[str], Optional[bool], float, float]:
    if (
        runtime.stop_event.is_set()
        and runtime.get_token_success_count() >= runtime.target_tokens
    ):
    ...
    email, mailbox_id = create_temp_email(
        session,
        email_api_base=runtime.email_api_base,
        email_domains=runtime.email_domains,
        api_key=runtime.email_api_key,
        expiry_time=runtime.email_expiry_time,
        logger=runtime.logger,
        retry_attempts=runtime.email_create_retry_attempts,
        retry_backoff_base=runtime.email_create_retry_backoff_base,
        retry_backoff_max=runtime.email_create_retry_backoff_max,
    )
    ...
    reg_ok = registrar.register(
        ...
        email_api_base=runtime.email_api_base,
        mailbox_id=mailbox_id,
        email_api_key=runtime.email_api_key,
        password=password,
    ...
    tokens = runtime.oauth_login_with_retry(
        email=email, password=password, mailbox_id=mailbox_id
    )
...
def run_batch_register(
    conf: Dict[str, Any], target_tokens: int, logger: logging.Logger
) -> tuple[int, int, int]:
    if target_tokens <= 0:
        return 0, 0, 0
    ...
    if not email_api_key:
        logger.error("email.api_key 未配置，无法调用 moemail 创建临时邮箱。")
        return 0, 0, 0

    runtime = RegisterRuntime(conf=conf, target_tokens=target_tokens, logger=logger)
    if not runtime.email_api_base:
        logger.error("email.base_url 未配置，无法调用 moemail API。")
        return 0, 0, 0

    workers = runtime.concurrent_workers

    logger.info(
        "开始补号: 目标 token=%s, 并发=%s, proxy=%s, email_api_base=%s, email_domains=%s, expiry_time=%s, create_retry_attempts=%s, create_retry_backoff_base=%s, create_retry_backoff_max=%s",
        target_tokens,
        workers,
        mask_proxy_for_log(runtime.proxy) or "-",
        runtime.email_api_base,
        ",".join(runtime.email_domains) or "<auto>",
        runtime.email_expiry_time,
        runtime.email_create_retry_attempts,
        runtime.email_create_retry_backoff_base,
        runtime.email_create_retry_backoff_max,
    )
    ...
    if workers == 1:
        ...
        def worker_task(task_index: int, worker_id: int):
            ...
            if (
                runtime.stop_event.is_set()
                and runtime.get_token_success_count() >= target_tokens
            ):
        ...
        try:
            ...
            while futures:
                ...
                done_set, _ = wait(
                    list(futures.keys()), return_when=FIRST_COMPLETED, timeout=1.0
                )
                if not done_set:
                    continue
...
async def run_probe_async(
    ...
    client_timeout = aiohttp_mod.ClientTimeout(total=max(1, timeout))
    ...
    async with aiohttp_mod.ClientSession(
...
def run_clean_401(conf: Dict[str, Any], logger: logging.Logger) -> tuple[int, int, int]:
    ...
    user_agent = str(
        pick_conf(conf, "clean", "user_agent", default=DEFAULT_MGMT_UA)
        or DEFAULT_MGMT_UA
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="账号池自动维护（三合一：清理+补号+收敛）"
    )
    ...
    parser.add_argument(
        "--timeout", type=int, default=15, help="统计 candidates 时接口超时秒数"
    )
...
def main() -> int:
    ...
    try:
        filled, failed, synced = run_batch_register(
            conf=conf, target_tokens=gap, logger=logger
        )
        logger.info(
            "补号阶段汇总: 成功token=%s, 失败=%s, 收敛账号=%s", filled, failed, synced
        )
    except Exception as e:
